...
import DeferredOnboardingBanner from "./DeferredOnboardingBanner";
...
// ─── Country coordinates ───────────────────────────────────────────────────────
// Extracted to `lib/countryMarkers.ts` (shared with onboarding's own VPN-
// destination resolution — see `onboarding-v2/lib/server.ts`); re-exported
// here so every existing import of `countryMarkers`/`CountryMarker` from
// this module keeps working unchanged.
export type { CountryMarker } from "../lib/countryMarkers";
export { countryMarkers };
...
// without a key still renders a map rather than failing outright. Kept as its
// own copy (not imported from onboarding-v2/lib/mapKit.ts) — v1 stays
// self-contained per the project's stated architecture.
...
interface WorldMapProps {
  ...
  onChangeServer?: () => void;
  /** Tier 3 of the connection-failure path — `null`/undefined hides the
   * banner entirely; `"retry"` (still disconnected) or `"resume"` (connected
   * later, personalization still unfinished) picks which copy/action shows.
   * Mutually exclusive with `showWelcomeBanner` in practice (the deferred
   * path never fires the success banner). See `DeferredOnboardingBanner`. */
  deferredOnboardingBannerMode?: "retry" | "resume" | null;
  onDeferredOnboardingAction?: () => void;
  onDeferredOnboardingDismiss?: () => void;
}
...
export function WorldMap({
  ...
  sessionPlan = "plus",
  onChangeServer,
  deferredOnboardingBannerMode = null,
  onDeferredOnboardingAction,
  onDeferredOnboardingDismiss,
}: WorldMapProps) {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      >
        ...
        {deferredOnboardingBannerMode && (
          <DeferredOnboardingBanner
            visible
            mode={deferredOnboardingBannerMode}
            onAction={() => onDeferredOnboardingAction?.()}
            onDismiss={() => onDeferredOnboardingDismiss?.()}
          />
        )}
      </div>
